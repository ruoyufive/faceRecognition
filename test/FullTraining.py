...
class Training(object):
    ...
    def train(self):
        ...
        model.add(Convolution2D(
            filters=5, #Output for next later layer output (100,100,32)
            kernel_size= (1,1) , #size of each filter in pixel
        ...
        #CNN Layer - 2
        model.add(Convolution2D(
            filters=10,  #Output for next layer (50,50,64)
            kernel_size=(2,2),
            padding='same',
        ))
        model.add(Activation('tanh'))
...
def MAIN():
    ...
    Train = Training(batch_size=128,number_batch=40,categories=4,train_folder='train_img/')
    Train.train()
